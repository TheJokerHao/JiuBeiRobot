...
public class LocalShopDetailsFragment extends BaseFragment {
    ...
    Button btnBuy;
    private String id;
    private LocalShopDetailsBean sDetailsBean;
...
//    private IdentityCheckDialog identityCheckDialog;
    ...
    public int pageLayout() {
        ...
        return R.layout.fragment_local_shop_details;
    }
    ...
    public void onResume() {
        ...
        if (id != null && id.length() > 0) {
            changeTab(0);
            //TODO  这里获取本地购买页面的详情
            APIManager.getInstance().localshopDetails(getContext(), this, id);
        }
    }
    ...
    public void onSuccess(BaseBean bean, int flag) {
        switch (flag) {
            case FLags.FLAG_LOCAL_SHOP_DETAILS:
                //添加语音播放可以在这里添加  判断获取的详情是否为空  要是为空就不播放  不为空就播放当前商品的详细信息
                BaseBean<ObjBean<LocalShopDetailsBean>> detailsBeanlocal = bean;
                sDetailsBean = detailsBeanlocal.getData().getObj();

                if (sDetailsBean != null) {
                    List<String> urls = new ArrayList<>();
                    if (BasicTool.isNotEmpty(sDetailsBean.getMainPic_url()))
                        urls.add(sDetailsBean.getMainPic_url());

                    if (BasicTool.isNotEmpty(sDetailsBean.getListPic1_url()))
                        urls.add(sDetailsBean.getListPic1_url());

                    if (BasicTool.isNotEmpty(sDetailsBean.getListPic2_url()))
                        urls.add(sDetailsBean.getListPic2_url());

                    if (BasicTool.isNotEmpty(sDetailsBean.getListPic3_url()))
                        urls.add(sDetailsBean.getListPic3_url());

                    if (BasicTool.isNotEmpty(sDetailsBean.getListPic4_url()))
                        urls.add(sDetailsBean.getListPic4_url());

                    if (BasicTool.isNotEmpty(sDetailsBean.getListPic5_url()))
                        urls.add(sDetailsBean.getListPic5_url());

                    if (urls != null) {
                        blShopBanner.setViewUrls(urls, null);
                    } else {
                        return;
                    }
...
//                ProductDescriptionH5 = HtmlRegexpUtil.getTextHTML(sDetailsBean.getProductDescriptionH5());
                    ...
                    ChateauHistory = HtmlRegexpUtil.getTextHTMLToCellarDesc(sDetailsBean.getChateauHistory());
                    Log.d("print", "商品详情页的酒庄历史: " + ChateauHistory);
                    ...
                    int stocklocal = sDetailsBean.getStock();
                    tvLsStock.setText(stocklocal + "瓶");
                    cnivNum.setMinVal(1);
                    cnivNum.setMax(stocklocal);
                    setCard();
                }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.iv_close:
                ...
                if (BaseApplication.currentMode == BaseApplication.LOCAL_BUY) {
                    if (getActivity() != null) {
                        ...
                        LocalShopDetailsFragment.this.onDestroy();
                    }
                } else {
                    if (getActivity() != null) {
                        ((LocalBuyActivity) getActivity()).closeShopDetails();
            ...
            case R.id.btn_buy:
                ...
                if (BaseApplication.currentMode == BaseApplication.LOCAL_BUY) {//购买方式为本地购买
                    //TODO  选择了本地购买 应该有些什么逻辑？
                    if (getActivity() != null) {
                        //((ShopListActivity) getActivity()).closeShopDetails();
                    }
                    Activitys.toLocalUserCheckOrder(getActivity(), LocalUserCheckOrderActivity.NOW_BUY, stockBean, "");
                    this.onDestroy();
            ...
            case R.id.btn_add_to_card:
                ...
                if (getActivity() != null && cnivNum.getCurentVal() > 0) {
                    ...
                    //TODO  加入购物车 需要判断
                    ((LocalBuyActivity) getActivity()).addToCard(ball, startLocation, sbean, cnivNum.getNum());
                }
                break;
//            case R.id.iv_guige:
//                //规格参数specification
