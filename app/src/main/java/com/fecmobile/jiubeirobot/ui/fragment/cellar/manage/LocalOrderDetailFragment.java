...
import com.fecmobile.jiubeirobot.ui.activity.cellar.manager.LocalOrderListActivity;
...
public class LocalOrderDetailFragment extends BaseFragment {
    ...
    private LocalOrderSendAndPayDialog orderSendAndPayDialog;
    ...
    public int pageLayout() {
        return R.layout.fragment_local_order_detail;
    }
    ...
    public void onResume() {
        ...
        if (orderId != null) {
            //TODO  获取item的详情
            APIManager.getInstance().getLocaOrderDetail(getActivity(), this, orderId);
        }
    }
    ...
    public void onSuccess(BaseBean bean, int flag) {
        switch (flag) {
            case FLags.FLAG_LOCAL_ORDER_DETAIL:
    ...
    private void initOrderDetail(ManagerOrderDetailBean bean) {
        ...
        List<ManagerOrderDetailBean.OrderlineListBean> orderlineList = bean.getOrderlineList();
        ...
        } else if (isHaveInvoice.equals("1")) {
            //不包含
    ...
    private void initProductLineList(List<ManagerOrderDetailBean.OrderlineListBean> orderlineList) {
        if (orderlineList == null || orderlineList.size() == 0) {
            return;
        }
        linProductList.removeAllViews();
        for (int i = 0; i < orderlineList.size(); i++) {
            ManagerOrderDetailBean.OrderlineListBean orderlineListBean = orderlineList.get(i);
    ...
    public void onClick() {
        if (orderStatus.equals("0")) {
            ...
            //打印小票
            if (getActivity() != null) {
                LocalOrderListActivity orderListActivity = (LocalOrderListActivity) getActivity();
    ...
    private void initSelectOperator() {
        if (orderSendAndPayDialog == null) {
            orderSendAndPayDialog = new LocalOrderSendAndPayDialog();
        }
    ...
    private void showConfirmDialog() {
        ...
        confirmDialog.setClickLisener(new ConfirmDropPointDialog.ClickLisener() {
            ...
            public void onLeftClick(AlertDialog dialog) {
                //取消
                APIManager.getInstance().setLocalOrderFinish(getActivity(), LocalOrderDetailFragment.this, orderId);
