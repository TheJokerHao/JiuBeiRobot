...
import com.fecmobile.jiubeirobot.ui.adapter.cellar.manage.LocalOrderListApater;
import com.fecmobile.jiubeirobot.ui.dialog.cellar.manage.LocalOrderSendAndPayDialog;
import com.fecmobile.jiubeirobot.ui.dialog.cellar.manage.SelectBluetoothAdapterDialog;
import com.fecmobile.jiubeirobot.ui.fragment.cellar.manage.LocalOrderDetailFragment;
...
public class LocalOrderListActivity extends BaseActivity implements XRecyclerView.LoadingListener, RadioGroup.OnCheckedChangeListener, SelectBluetoothAdapterDialog.SelectDriverLinsener, ItemButtonClick, DrawerLayout.DrawerListener {
    ...
    private LocalOrderDetailFragment orderDetailFragment;
    ...
    private LocalOrderListApater mAdapter;
    private String searchContent;

    private LocalOrderSendAndPayDialog sendAndPayDialog;
...
//        dlOrderList.setDrawerLockMode(DrawerLayout.LOCK_MODE_LOCKED_CLOSED);
        ...
        mAdapter = new LocalOrderListApater(this, mData);
    ...
    public void onSuccess(BaseBean bean, int flag) {
        switch (flag) {
            case FLags.FLAG_LOCAL_ORDER_LIST:
                ...
                dismissHUD();
                break;
            case FLags.FLAG_LOCAL_ORDER_FINISH:
                //刷新数据
                rvOrderList.refresh();
                ...
                break;
            case FLags.FLAG_LOCAL_ORDER_RECEIABLE:
                //订单收款
                rvOrderList.refresh();
                if (orderDetailFragment != null) {
                    orderDetailFragment.onResume();
                ...
                break;
            case FLags.FLAG_LOCAL_ORDER_SEND:
                //订单发货
                rvOrderList.refresh();
                if (orderDetailFragment != null) {
                    orderDetailFragment.onResume();
                ...
                break;
            case FLags.FLAG_LOCAL_RECEIPT_INFO:
                //打印小票
...
//        tvFinishMoney.setText(Html.fromHtml(totalMoney));
    ...
    private void setLoading() {
        APIManager.getInstance().getLocalOrderList(this, filterStatus, timeSection, pageUtils.getPageIndex(), pageUtils.getPageSize(), searchContent);
    }
    ...
    private void addOrderDetailFragment(int position) {
        if (orderDetailFragment == null) {
            orderDetailFragment = new LocalOrderDetailFragment();
    ...
    public void onItemButtonClick(int position, Object type) {
        ...
        L.i(method);
        Log.d("print", "这是对订单状态的处理接收到的数据 : " + method);
        ...
        if (method.equals("item")) {
            ...
            APIManager.getInstance().setLocalOrderFinish(this, this, orderListBean.getId());
        } else if (method.equals("receive") || method.equals("send")) {
            //收款和发货操作
            sendAndPayDialog = new LocalOrderSendAndPayDialog();
    ...
    public void print(int position) {
        ...
        if (bluetoos.getService(this).getState() != BluetoothService.STATE_CONNECTED) {
            ...
            APIManager.getInstance().getLocalReceiptByOrderID(this, mData.get(position).getId());
        }
    }
